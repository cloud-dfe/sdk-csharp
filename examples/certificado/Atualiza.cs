...
var certificado = new Certificado(config);

try
{

    string xmlFile = Util.ReadFile("caminho_do_arquivo.pfx");

    string xmlFileBase64 = Util.Encode(xmlFile);

    var payload = new Dictionary<string, object>
    {
        {"certificado", xmlFileBase64},
        {"senha", "senha"},
    };

    var resp = Task.Run(async () => await certificado.Atualiza(payload)).GetAwaiter().GetResult();
