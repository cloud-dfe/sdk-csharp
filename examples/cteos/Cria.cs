...
try
{
    ...
    var resp = Task.Run(async () => await cteos.Cria(payload)).GetAwaiter().GetResult();
    ...
    if (resp.ContainsKey("sucesso") && (bool)resp["sucesso"])
    {
        ...
        while (tentativa <= 5)
        {
            ...
            var respC = Task.Run(async () => await cteos.Consulta(payloadConsulta)).GetAwaiter().GetResult();
